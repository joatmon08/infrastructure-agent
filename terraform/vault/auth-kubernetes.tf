...
resource "vault_kubernetes_auth_backend_role" "client_agents" {
  ...
  token_ttl                        = 3600
  token_policies                   = [vault_policy.actor_token[each.key].name, vault_policy.agent_oidc_client.name, vault_policy.oauth_exchange_token[each.key].name, vault_policy.vault_token_create.name]
}