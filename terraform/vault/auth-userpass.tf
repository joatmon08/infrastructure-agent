...
# Test user for token creation testing
ephemeral "random_password" "test_user" {
  length  = 16
  special = false
}

resource "vault_kv_secret_v2" "test_user_password" {
  mount = vault_mount.credentials.path
  name  = "test-user"
  data_json_wo = jsonencode({
    username = "test-user"
    password = ephemeral.random_password.test_user.result
  })
}

data "vault_kv_secret_v2" "test_user_password" {
  mount = vault_kv_secret_v2.test_user_password.mount
  name  = vault_kv_secret_v2.test_user_password.name
}

resource "vault_generic_endpoint" "test_user" {
  path                 = "auth/${vault_auth_backend.userpass.path}/users/test-user"
  ignore_absent_fields = true
  data_json            = <<EOT
{
  "token_policies": ["test-client-vault-token-create"],
  "token_ttl": "1h",
  "password": "${data.vault_kv_secret_v2.test_user_password.data.password}"
}
EOT
}
...
resource "vault_identity_entity_alias" "test_user" {
  name           = "test-user"
  mount_accessor = vault_auth_backend.userpass.accessor
  canonical_id   = vault_identity_entity.test_user.id
}