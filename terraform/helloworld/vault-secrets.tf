...
# VaultDynamicSecret for Vault token (using token role)
# This token is only used for STS token exchange, so it only needs the oauth-exchange-token policy
resource "kubernetes_manifest" "vault_token" {
  manifest = {
    apiVersion = "secrets.hashicorp.com/v1beta1"
    kind       = "VaultDynamicSecret"
    metadata = {
